...
def main():
    ...
    for ds in dataset_names:
        ...
        for fold in range(1, 6):
            ...
            test_labels = [1] * len(test_pos_pairs) + [0] * len(test_neg_pairs)
            ...
            model_path = os.path.join(ds_output_folder, f"rf_model_{ds}_fold{fold}.pkl")
            joblib.dump(rf_model, model_path)

        results_df = pd.DataFrame(fold_results)
        results_csv = os.path.join(ds_output_folder, f"fold_results_{ds}.csv")
        results_df.to_csv(results_csv, index=False)
        print(f"{ds} 每折结果已保存至: {results_csv}")
