...
def train_and_evaluate_supervised(model, train_data, test_data, optimizer, num_epochs=500):
    ...
    for epoch in range(num_epochs):
        ...
        pos_out = model.edge_decoder(z, train_data.pos_edge_label_index)
        neg_out = model.edge_decoder(z, train_data.neg_edge_label_index)
        ...
        model.eval()
        with torch.no_grad():
            z_eval = model.encoder(test_data.x, test_data.edge_index)
            metrics = model.test(z_eval, test_data.pos_edge_label_index, test_data.neg_edge_label_index)
        if metrics[2] > best_acc:
            best_acc = metrics[2]
            best_state = copy.deepcopy(model.state_dict())
            best_metrics = metrics
        if (epoch + 1) % 100 == 0:
            print(f"Supervised Epoch {epoch + 1:03d}: Test ACC={metrics[2]:.4f}")
    train_time = time.time() - start_time
    return best_metrics, best_state, train_time
...
def train_and_evaluate(model, train_data, test_data, optimizer, num_epochs=500, alpha=0.4):
    ...
    for epoch in range(num_epochs):
        ...
        if metrics[2] > best_acc:
            ...
            best_metrics = metrics
        if (epoch + 1) % 100 == 0:
            print(f"Self-Supervised Epoch {epoch + 1:03d}: Test ACC={metrics[2]:.4f}")
    train_time = time.time() - start_time
    return best_metrics, best_state, train_time
...
if __name__ == '__main__':
    ...
    for fold in range(1, 6):
        ...
        for regime in ["Self-Supervised", "Supervised"]:
            ...
            if regime == "Self-Supervised":
                metrics, best_state, train_time = train_and_evaluate(model, train_data_rand, test_data_rand, optimizer)
            else:  # regime == "Supervised"
                metrics, best_state, train_time = train_and_evaluate_supervised(model, train_data_rand, test_data_rand, optimizer)
            ...
            fold_dir = os.path.join(save_path, regime, f"fold{fold}")
            os.makedirs(os.path.join(fold_dir, "train"), exist_ok=True)
            os.makedirs(os.path.join(fold_dir, "test"), exist_ok=True)
            torch.save(train_data_rand.x, os.path.join(fold_dir, "train", "x.pt"))
            torch.save(train_data_rand.edge_index, os.path.join(fold_dir, "train", "edge_index.pt"))
            torch.save(train_data_rand.pos_edge_label_index, os.path.join(fold_dir, "train", "pos_edge_label_index.pt"))
            torch.save(train_data_rand.neg_edge_label_index, os.path.join(fold_dir, "train", "neg_edge_label_index.pt"))
            torch.save(test_data_rand.x, os.path.join(fold_dir, "test", "x.pt"))
            torch.save(test_data_rand.edge_index, os.path.join(fold_dir, "test", "edge_index.pt"))
            torch.save(test_data_rand.pos_edge_label_index, os.path.join(fold_dir, "test", "pos_edge_label_index.pt"))
            torch.save(test_data_rand.neg_edge_label_index, os.path.join(fold_dir, "test", "neg_edge_label_index.pt"))
            train_pairs_file = os.path.join(fold_dir, "train", "train_pairs.txt")
            save_pairs(train_data_rand.pos_edge_label_index, train_pairs_file)
            ...
            print(f"[{regime}] Fold {fold} results: ACC={metrics[2]:.4f}, AUC={metrics[0]:.4f}, Train Time={train_time:.2f}s")


    results_df = pd.DataFrame(results_all)
    results_csv_path = os.path.join(save_path, f"supervised_vs_selfsupervised_results.csv")
    results_df.to_csv(results_csv_path, index=False)
    print(f"\n✅ 所有折与训练方式的对比结果已保存到: {results_csv_path}")
